...
public class UserStoreDeployer extends IdentityServerDeployer {
    ...
    protected void doDeploy(IdentityServerArtifact identityServerArtifact, Server server)
        ...
        UserStoreDTO userStoreDTO = ResourceManager.loadXMLToObject(
                identityServerArtifact.getAbsoluteResourcePath(), UserStoreDTO.class);
        if (userStoreDTO != null) {
            try {

                UserStoreDTO[] secondaryRealmConfigurations = IdentityServerAdminClient
                        .getUserStoreConfigAdminService(server).getSecondaryRealmConfigurations();
                if (secondaryRealmConfigurations != null) {
                    for (UserStoreDTO secondaryRealmConfiguration : secondaryRealmConfigurations) {
                        if (secondaryRealmConfiguration != null && secondaryRealmConfiguration.getDomainId().equals
                                (userStoreDTO.getDomainId())) {
                            IdentityServerAdminClient.getUserStoreConfigAdminService(server).
                                    deleteUserStore(userStoreDTO.getDomainId());
                            break;
                        }
                    }
                }
                if (!isUserStoreRemoved(server, userStoreDTO, 0)) {
                    throw new CarbonSolutionException("Error occurred while removing user store, " +
                            identityServerArtifact.getResourcePath());
                }

                cleanDB(userStoreDTO, identityServerArtifact);
                buildH2DB(userStoreDTO, identityServerArtifact);
                IdentityServerAdminClient.getUserStoreConfigAdminService(server).addUserStore(userStoreDTO);

                //Waiting to user store deployment.
                if (!isUserStoreExists(server, userStoreDTO, 0)) {
                    throw new CarbonSolutionException(
                            "User store doesn't not installed, " + userStoreDTO.getDomainId());
                }
            } catch (Exception e) {
                throw new CarbonSolutionException("Error occurred while deploying user store, " +
                        identityServerArtifact.getResourcePath(), e);
            }
        }
    }
        ...
        try {
            ...
            if (secondaryRealmConfigurations != null) {
                for (UserStoreDTO secondaryRealmConfiguration : secondaryRealmConfigurations) {
                    try {
                        IdentityServerAdminClient.getUserStoreConfigAdminService(server).
                                deleteUserStore(secondaryRealmConfiguration.getDomainId());
                    } catch (Exception e) {
                        log.error("Error occurred while deleteUserStore, " +
                                secondaryRealmConfiguration.getDomainId(), e);
                    }
                }
            }
        } catch (Exception e) {
            log.error("Error occurred while getSecondaryRealmConfigurations, " + e.getMessage());
        }
    }

    /**
     * Check user store is removed or not from the server.
     *
     * @param server
     * @param userStoreDTO
     * @param count
     ...
     */
    private boolean isUserStoreRemoved(Server server, UserStoreDTO userStoreDTO, int count)
            throws RemoteException, UserStoreConfigAdminServiceIdentityUserStoreMgtException, CarbonSolutionException,
        ...
        if (secondaryRealmConfigurations != null) {
            for (UserStoreDTO secondaryRealmConfiguration : secondaryRealmConfigurations) {
                if (secondaryRealmConfiguration != null && secondaryRealmConfiguration.getDomainId().equals
                        (userStoreDTO.getDomainId())) {
                    if (count < 30) {
                        log.info("Checking user store removed for domain : " + userStoreDTO.getDomainId());
                        Thread.sleep(1000);
                        return isUserStoreRemoved(server, userStoreDTO, ++count);
                    } else {
                        log.info("Timeout to validate the user store status, " + userStoreDTO.getDomainId());
                        return false;
                    }
                }
            }
        }
        log.info("User store was removed, " + userStoreDTO.getDomainId());
        return true;
    }

    private boolean isUserStoreExists(Server server, UserStoreDTO userStoreDTO, int count)
            throws RemoteException, UserStoreConfigAdminServiceIdentityUserStoreMgtException, CarbonSolutionException,
            InterruptedException {
        ...
        log.info("Checking user store installed for  " + userStoreDTO.getDomainId());
        Thread.sleep(1000);
        return isUserStoreExists(server, userStoreDTO, ++count);
    }
     ...
     *
     * @param userStoreDTO
     * @param identityServerArtifact
     * @throws CarbonSolutionException
     */
    private void buildH2DB(UserStoreDTO userStoreDTO, IdentityServerArtifact identityServerArtifact)
            throws CarbonSolutionException {
        ...
        for (PropertyDTO property : properties) {
            if (property.getName().equals("url")) {
                ...
                if (StringUtils.isEmpty(value)) {
                    String absoluteArtifactHomePath = identityServerArtifact.getAbsoluteArtifactHomePath();
                    String h2DBUrl = "jdbc:h2:" + absoluteArtifactHomePath + userStoreDTO.getDomainId();
                    property.setValue(h2DBUrl);
                    cloneH2DB(userStoreDTO.getDomainId(), absoluteArtifactHomePath);
                    break;
                }
            }
        }
    ...
    /**
     * Clone H2 Database for given solution.
     *
     * @param dbName
     * @param destination
     * @throws CarbonSolutionException
     */
    private void cloneH2DB(String dbName, String destination)
            throws CarbonSolutionException {

        dbName = dbName + ".h2.db";
        String absoluteSourceDBPath = ApplicationUtility.getCommonDataShemaHome() + File.separator +
                Constant.CommonResource.H2_DB;
        String absoluteDestDBPath = destination + File.separator + dbName;
        try {
            FileUtils.copyFile(new File(absoluteSourceDBPath), new File(absoluteDestDBPath));
        } catch (IOException e) {
     ...
     * @throws CarbonSolutionException
     */
    private void cleanDB(UserStoreDTO userStoreDTO, IdentityServerArtifact identityServerArtifact)
            throws CarbonSolutionException {

        String dbName = userStoreDTO.getDomainId();
        File dir = new File(identityServerArtifact.getAbsoluteArtifactHomePath());
        ...
        if (files != null) {
            for (File file : files) {
                try {
                    ...
                    log.error("Error occurred while deleting, " + file.getAbsolutePath(), e);
                }
            }
        }
    }

    protected void doDeploy2(IdentityServerArtifact identityServerArtifact, Server server)
            throws CarbonSolutionException {
