...
public class SAML2ConfigDeployer {
    ...
    public static void deploy(ServiceProvider serviceProvider,
        ...
        if (inboundAuthenticationConfig != null) {
            ...
            if (inboundAuthenticationRequestConfigs != null) {
                for (InboundAuthenticationRequestConfig inboundAuthenticationRequestConfig :
                    ...
                    if (PROTOCOL_SAML.equals(inboundAuthenticationRequestConfig.getInboundAuthType())) {
                        SAMLSSOServiceProviderDTO samlssoServiceProviderDTO = ResourceManager.loadXMLToObject(
                                identityServerArtifact.getAbsoluteResourcePath(), SAMLSSOServiceProviderDTO.class);
                        inboundAuthenticationRequestConfig.setInboundAuthKey(samlssoServiceProviderDTO.getIssuer());
                        samlSSOServiceProviderInfoDTO_server =
                                getSAMLSSOServiceProviderInfoDTO(
                                        inboundAuthenticationRequestConfig.getInboundAuthKey(), server);
                        if (samlSSOServiceProviderInfoDTO_server != null) {
                            try {
                                IdentityServerAdminClient.getSAMLSSOConfigService(server)
                                        .removeServiceProvider(samlssoServiceProviderDTO.getIssuer());
                            } catch (Exception e) {
                                log.warn("Error occurred while getting removeServiceProvider/addRPServiceProvider, " + e
                                        .getMessage());
                            }
                        }
                        IdentityServerAdminClient.getSAMLSSOConfigService(server)
                                .addRPServiceProvider(samlssoServiceProviderDTO);
                        samlSSOServiceProviderInfoDTO_server = getSAMLSSOServiceProviderInfoDTO(
                                inboundAuthenticationRequestConfig.getInboundAuthKey(), server);
                        break;
                    }
                }
        ...
        ApplicationUtility.updateProperty(identityServerArtifact, samlProperties.getSamlProperties());
    }
    ...
    public static SAMLSSOServiceProviderDTO getSAMLSSOServiceProviderInfoDTO(String issuer, Server server) {
        ...
        try {
            SAMLSSOServiceProviderInfoDTO serviceProviders = IdentityServerAdminClient.getSAMLSSOConfigService(server)
                    .getServiceProviders();
            SAMLSSOServiceProviderDTO[] serviceProvidersDTOs = serviceProviders.getServiceProviders();
            for (SAMLSSOServiceProviderDTO serviceProviderDTO : serviceProvidersDTOs) {
                if (serviceProviderDTO.getIssuer().equals(issuer)) {
                    samlssoServiceProviderDTO = serviceProviderDTO;
                }
            }
        } catch (Exception e) {
            log.warn("Error occurred while getSAMLSSOServiceProviderInfoDTO, " + e.getMessage());
        }
