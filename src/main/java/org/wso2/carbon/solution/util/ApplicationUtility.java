...
public class ApplicationUtility {
            ...
            if (fileObj.exists()) {
                if (fileObj.isDirectory()) {
                    try {
                        ...
                        log.error("Error occurred while deleting a folder, " + file, e);
                    }
                } else {
                    try {
                        FileUtils.forceDelete(fileObj);
                    } catch (IOException e) {
                        log.error("Error occurred while deleting a file, " + file, e);
                    }
                }
            }
        }
    ...
    public static String getBase64EncodedBasicAuthHeader(String userName, String password)
        ...
        try {
            ...
            throw new CarbonSolutionException("Error occurred while reading credential, ", e);
        }
